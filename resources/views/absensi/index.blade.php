    ...
    <div class="ml-64 p-10 min-h-screen bg-gray-50">
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('absensi.create') }}"
                class="bg-black text-white px-6 py-3 rounded-lg font-bold text-sm hover:bg-gray-800 shadow-lg">
                + Input Kehadiran
            </a>
        </div>
                ...
                <div class="flex items-center gap-4">
                    <div class="flex items-center gap-2">
                        ...
                        <select name="limit" onchange="this.form.submit()"
                            class="border-gray-300 rounded text-sm bg-gray-50 focus:ring-black focus:border-black">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <select name="kelas" onchange="this.form.submit()"
                            class="border-gray-300 rounded text-sm bg-gray-50 focus:ring-black focus:border-black min-w-[100px]">
                            <option value="">Semua</option>
                            @foreach ($datakelas as $dk)
                            <option value="{{ $dk->nama_kelas }}"
                                {{ $kelasSelected == $dk->nama_kelas ? 'selected' : '' }}>{{ $dk->nama_kelas }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="relative">
                        <input type="text" name="search" value="{{ $search }}" placeholder="Cari Siswa..."
                            class="pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm w-64 focus:ring-black focus:border-black">
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <table class="w-full text-left border-collapse">
            <thead class="bg-gray-100 border-b border-gray-200">
                <tr>
                    <th
                        class="p-4 text-xs font-bold text-gray-500 uppercase cursor-pointer hover:bg-gray-200 transition select-none">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="p-4 text-center flex justify-center gap-2">
                        <a href="{{ route('absensi.edit', $d->id_kehadiran) }}"
                            class="bg-blue-50 text-blue-600 px-3 py-1 rounded text-xs font-bold hover:bg-blue-100">Edit</a>
                        @if(auth()->user()->role == 'admin')
                        <form action="{{ route('absensi.destroy', $d->id_kehadiran) }}" method="POST"
                            onsubmit="return confirm('Hapus?');">
                            @csrf @method('DELETE')
                            <button
                                class="bg-red-50 text-red-600 px-3 py-1 rounded text-xs font-bold hover:bg-red-100">Hapus</button>
                        </form>
                        @endif
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="6" class="p-8 text-center text-gray-400">Data tidak ditemukan.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
