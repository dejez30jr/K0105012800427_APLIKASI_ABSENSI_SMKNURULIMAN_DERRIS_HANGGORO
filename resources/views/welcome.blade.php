...
<body class="font-sans antialiased">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16 items-center">
                <div class="flex items-center">
                    <h1 class="text-2xl font-black text-gray-500 tracking-tighter">ABSENSI DIGITAL</h1>
                </div>
                <div>
                    @if (Route::has('login'))
                    @auth
                    <a href="{{ url('/dashboard') }}" class="font-bold text-white hover:text-black">Dashboard</a>
                    @else
                    <a href="{{ route('login') }}"
                        class="bg-blue-500 text-white px-5 py-2 rounded-lg font-bold text-sm transition shadow-lg hover:bg-blue-600">
                        Masuk
                    </a>
                    @endauth
                    @endif
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-10">
        ...
        <div class="text-center mb-10">
            <h2 class="text-4xl font-black text-black mb-2">Kehadiran Hari Ini</h2>
            ...
            <form method="GET" action="{{ route('home') }}"
                ...
                <div class="flex items-center gap-4 w-full md:w-auto">
                    <div class="flex items-center gap-2">
                        ...
                        <select name="limit" onchange="this.form.submit()"
                            class="border-gray-300 rounded-lg text-sm bg-gray-50 focus:ring-black focus:border-black p-2">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <select name="kelas" onchange="this.form.submit()"
                            class="border-gray-300 rounded-lg text-sm bg-gray-50 focus:ring-black focus:border-black p-2 min-w-[120px]">
                ...
                <div class="w-full md:w-auto relative">
                    <input type="text" name="search" value="{{ $search }}" placeholder="Cari Nama / NIS..."
                        class="w-full md:w-64 pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-black focus:border-black">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition duration-150">
                        ...
                        <td class="p-4 text-sm text-gray-600">
                            <span
                                class="bg-gray-100 border border-gray-200 px-2 py-1 rounded text-xs font-bold">{{ $d->id_kelas }}</span>
                        </td>
                        <td class="p-4 text-center">
                            @if($d->kehadiran == 'Hadir')
                            <span
                                class="bg-green-100 text-green-700 px-3 py-1 rounded-full text-xs font-bold shadow-sm">Hadir</span>
                            @elseif($d->kehadiran == 'Terlambat')
                            <span
                                class="bg-yellow-100 text-yellow-700 px-3 py-1 rounded-full text-xs font-bold shadow-sm">Terlambat</span>
                            @else
                            <span
                                class="bg-red-100 text-red-700 px-3 py-1 rounded-full text-xs font-bold shadow-sm">{{ $d->kehadiran }}</span>
                            @endif
                        </td>
                    </tr>
                    @empty
    ...
    <div class="text-center py-6 text-white text-sm">
        &copy; {{ date('Y') }} Absensi Digital
    </div>
